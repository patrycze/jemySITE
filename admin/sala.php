...
<html>
	...
	<body>
		...
		<form action="updateReservation.php" id="main_form" class="form-horizontal" method="post" >
						<div class="form-group">
					        	<label class="col-sm-4 control-label" for="food_1">Rezerwacje</label>
				        		<div class="col-sm-8">
									<select id="food_1_1" class="form-control" name="food_1_1">
										<option value="0">Wybierz rezerwacje do usunięcia</option>
										<?php
										require 'config.php';

										$sql = "SELECT stolik_id, DataRezerwacji, GodzinaRezerwacji from rezerwacja where Status = '1'";
										$result = mysqli_query($conn, $sql);

										if (mysqli_num_rows($result) > 0) {
										    // output data of each row
										    while($row = $result->fetch_assoc()) {	
										echo "<option value=\"" .$row['DataRezerwacji'] . "\">" ."Data: ". $row['DataRezerwacji']," Godzina: ". $row['GodzinaRezerwacji'],
											" Stolik: ". $row['stolik_id']										. "</option>";
							    }
										} else {
